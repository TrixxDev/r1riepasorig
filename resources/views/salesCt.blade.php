                                            ...
                                            <div style="width: 100%">
                                                <div class="form-group mb-0">
                                                    <select name="category" class="r1-select select-title tire-category">
                                                        <option class="select-list" value="Visi">Visi</option>
                                                        @foreach ($categories as $category_id => $category)
                                                            <option class="select-list" @if ($categoryId===$category_id){{'selected'}}@endif value="{{ $category_id }}">{{ ucwords($category['name']) }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
