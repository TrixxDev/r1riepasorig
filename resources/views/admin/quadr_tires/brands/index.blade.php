...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="fade-in">
            <div class="card">
                <div class="card-header"> Kvadraciklu riepu brendi
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-6">
                                <div class="dataTables_length" id="DataTables_Table_0_length">
                                    <label>Rādīt
                                        <select name="perPage" onchange="location = '{{ route('admin.quadr.brands.search') }}/' + this.value" aria-controls="DataTables_Table_0" class="custom-select custom-select-sm form-control form-control-sm">
                                    ...
                                    @if (Session::get('search'))
                                        <a class="btn btn-md btn-primary" style="margin-left: 100px;" href="{{ route('admin.quadr.brands.search') }}">Dzēst filtru</a>
                                    @endif
                                </div>
                            </div>
                            <div class="col-sm-12 col-md-6" style="display: inline-flex; place-content: flex-end;">
                                <form method="post" action="{{ route('admin.quadr.brands.search') }}">
                                    @csrf
                                ...
                                <a style="float: right; height: 28.38px; line-height: 13px; margin-left: 10px;" class="btn btn-md btn-primary" href="{{ route('admin.quadr.brands.add') }}"> Pievienot brendu</a>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-12">
...
{{--                                            <td>{{ $tire->d1 }}</td>--}}
                                            <td style="width: 8.3%!important;">
                                                <a class="btn btn-success" href="{{ route('admin.quadr.brands.edit', $brand->brand_id) }}">
                                                    <svg class="c-icon">
                                                        <use xlink:href="{{ asset('admins/assets/coreui-icons/sprites/free.svg') }}#cil-description"></use>
                                                    </svg>
                                                </a>
                                                <a class="btn btn-danger brand_delete" id="{{ $brand->brand_id }}" href="{{ route('admin.quadr.brands.delete', $brand->brand_id) }}">
                                                    <svg class="c-icon">
