...
@section('content')
    ...
    <div class="container-fluid">
        <div class="fade-in">
            <div class="card">
                <form class="form-horizontal" action="{{ route('admin.quadr.treads.update', $tread->tread_id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="card-body">
                            ...
                            <div class="col-md-9">
                                <select name="tread_brand" class="form-control" id="brand-input">
                                    @foreach ($brands as $brand)
                                        <option value="{{ $brand->brand_id }}" @if ($brand->brand_id == $tread->brand_id) {{ 'selected' }} @endif >{{ strtoupper($brand->title) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="preview-image col-md-9">
                                {!! \App\Helper\Image::showGrid('quadr', $tread->tread_id, 'width: 300px; height: 300px;') !!}
                            </div>
                        </div>
                    </div>
                    <div class="card-footer">
                        <button class="btn btn-md btn-primary" type="submit"> Saglabāt</button>
                        <a class="btn btn-md btn-info" href="{{ route('admin.quadr.treads') }}"> Atpakaļ</a>
                    </div>
                </form>
            </div>
        </div>
