...
@section('content')
    ...
    <div class="container-fluid">
        ...
        <div class="fade-in">
            <div class="card">
                <div class="card-header"> Kvadraciklu riepu modeļi
                        ...
                        <div class="row">
                            <div class="col-sm-12 col-md-6">
                                <div class="dataTables_length" id="DataTables_Table_0_length">
                                    <label>Rādīt
                                        <select name="perPage" onchange="location = '{{ route('admin.quadr.treads.search') }}/' + this.value" aria-controls="DataTables_Table_0" class="custom-select custom-select-sm form-control form-control-sm">
                                    ...
                                    @if (Session::get('search'))
                                        <a class="btn btn-md btn-primary" style="margin-left: 100px;" href="{{ route('admin.quadr.treads.search') }}">Dzēst filtru</a>
                                    @endif
                                </div>
                            </div>
                            <div class="col-sm-12 col-md-6" style="display: inline-flex; place-content: flex-end;">
                                <form method="post" action="{{ route('admin.quadr.treads.search') }}">
                                    @csrf
                                ...
                                <a style="float: right; height: 28.38px; line-height: 13px; margin-left: 10px;" class="btn btn-md btn-primary" href="{{ route('admin.quadr.treads.add') }}"> Pievienot modeli</a>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-sm-12">
                                <table class="table table-striped table-bordered datatable dataTable no-footer" id="DataTables_Table_0" role="grid" aria-describedby="DataTables_Table_0_info" style="border-collapse: collapse !important">
                                    ...
                                    @foreach ($treads as $tread)
                                        <tr role="row" class="odd">
                                            <td class="sorting_1">{{ ucwords($tread->title) }}</td>
                                            {{--                                            <td>{{ $tire->d1 }}</td>--}}
                                            <td style="width: 8.3%!important;">
                                                <a class="btn btn-success" href="{{ route('admin.quadr.treads.edit', $tread->tread_id) }}">
                                                    <svg class="c-icon">
